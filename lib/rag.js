 ...
 */
export const DEFAULT_DENY_CONTENT = [
...
function compileDenyPatterns(patterns) {
    return patterns.map((src) => {
        try {
            ...
            throw new Error(`rag: invalid denyContent pattern ${JSON.stringify(src)}: ${err.message}`);
        }
    });
}
/**
 ...
 */
export function chunkMarkdown(text, fileTitle, denyContent) {
...
function chunkMarkdownImpl(text, fileTitle) {
...
export class RagEngine {
    ...
    vec0FailureState(db, rowid) {
        try {
            ...
            const shadowMax = db
                ...
                .get();
            const seqRow = db
                .prepare("SELECT COALESCE(seq, 0) AS seq FROM sqlite_sequence WHERE name = 'chunks_rowids'")
                .get();
            const existing = db
                .prepare("SELECT chunk_id FROM chunks_rowids WHERE rowid = ?")
                .get(rowid);
            const shadowCount = db.prepare("SELECT count(*) AS c FROM chunks_rowids").get();
            const chunkCount = db.prepare("SELECT count(*) AS c FROM chunks").get();
            const gap = Math.max(0, Number(shadowCount.c) - Number(chunkCount.c));
            return (`rowid=${rowid} max_rowid=${maxRow.m} shadow_max=${shadowMax.m} ` +
     ...
     */
    async ensureIndex(databases, opts = {}) {
        ...
        try {
            ...
            if (this.indexRun === run)
                this.indexRun = undefined;
        }
    }
    /** The actual index pass (serialized by {@link ensureIndex}). */
    async runEnsureIndex(databases, clear) {
            ...
            for (const database of databases) {
                ...
                for (const file of files) {
                    ...
                    if (chunks.length > 0) {
                        ...
                        let inserted = false;
                        for (let attempt = 1; attempt <= MAX_INDEX_FILE_ATTEMPTS && !inserted; attempt++) {
                            db.exec("BEGIN");
                            let lastRowid = 0n;
                            try {
                                ...
                                const ctx = `${database.name}/${file.path}: ${err.message}` +
                                    (diag ? ` [${diag}]` : "");
                                if (attempt >= MAX_INDEX_FILE_ATTEMPTS) {
                                    this.logger(`rag: skipping ${ctx} (persistent per-file failure; index ages, boot pass continues)`);
                                }
                                else {
                                    ...
                                    try {
                                        ...
                                        const seqRow = db
                                            .prepare("SELECT COALESCE(seq, 0) AS seq FROM sqlite_sequence WHERE name = 'chunks_rowids'")
                                            .get();
                                        const floor = BigInt(Math.max(Number(shadowRow.max_rowid), Number(seqRow?.seq ?? 0)));
                                        if (nextRowid <= floor)
                                            nextRowid = floor + 1n;
                                    }
                                    catch {
 ...
 */
async function walkMarkdown(root, fs, path, opts = {}) {
